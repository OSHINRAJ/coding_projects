...
{	
	...
	makePixelArray();
	for (i = 0; i < height; i++)
		inFile.read(reinterpret_cast<char *>(Pixel[i]), width*sizeof(uint8_t));
	inFile.close();
}
...
void Image::writeImage(char file_name[])
{
	...
	for (i = 0; i < height; i++)
		outFile.write(reinterpret_cast<char *>(Pixel[i]), width*sizeof(uint8_t));
	outFile.close();
}
